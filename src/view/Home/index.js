    ...
    return (
        <>
            <div className='flex justify-between'>
                ...
                <Buttons onClick={handlePostData} className={"flex justify-between items-center"}>
                ...
                <>
                    {data.length > 0 ? (
                        <div className='mt-14 grid grid-cols-4 gap-4'>
                            {data.map((item) => (
                                <Card name={item.title} date={item.date}
                                    onDelete={() => {
            ...
            <Modal footer positionFooter={"center"} btn_2={"Hapus"} btn_1={"Batal"} view={viewModal} handleSubmit={() => handleDeleteData(dataItem?.id)} handleClose={() => setViewModal(!viewModal)}>
                {/*body*/}
                <div className="relative p-1">
                    ...
                    <div>
                        <p className='text-lg text-center mb-8'>
                            Apakah anda yakin menghapus activity <span className='font-bold'>“{dataItem?.title}“</span>?
                        </p>
                    </div>
                </div>
            </Modal>
            <Modal width={"sm"} view={viewAlert} handleClose={() => setViewAlert(!viewAlert)}>
                {/*body*/}
                <div className="">
